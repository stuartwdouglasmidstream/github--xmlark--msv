...
export PROJECT_VERSION=2013.6.1
export JAVA_HOME=/lib/jvm/java-1.7.0
export ENFORCE_VERSION=true

set -- "$@" install -DallowIncompleteProjects -Danimal.sniffer.skip -Dcheckstyle.skip -Dcobertura.skip -Denforcer.skip -Dformatter.skip -Dgpg.skip -Dimpsort.skip -Djapicmp.skip -Dmaven.javadoc.failOnError=false -Dmaven.site.deploy.skip -Dpgpverify.skip -Drat.skip=true -Drevapi.skip -Dsort.skip -Dspotbugs.skip -Dspotless.check.skip=true -DskipTests -Prelease org.apache.maven.plugins:maven-deploy-plugin:3.0.0-M2:deploy 
...
if [ -n "true" ]; then
  echo "Setting version to 2013.6.1 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="2013.6.1" | tee /root/project/logs/enforce-version.log
fi
