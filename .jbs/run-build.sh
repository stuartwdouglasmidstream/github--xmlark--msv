...
if [ -n "" ]; then
  echo "Setting version to 2022.7 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="2022.7" | tee /root/project/logs/enforce-version.log
fi
